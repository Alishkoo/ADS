...
bool spec_rabinKarp(string s, string t, vector<long long> hashes, long long smallHash)
{
    for (long long i = 0; i < s.size() - t.size() + 1; i++)
    {
        long long hashDif = hashes[i + t.size() - 1];
...
int main()
{
    ...
    // for (auto i : to_check_small) { cout << i << " "; }
